document.getElementById("analysis-button").addEventListener("click", function () {
    browser.tabs.query({ active: true, currentWindow: true }, function (tabs) {
        browser.tabs.sendMessage(tabs[0].id, { from: "popup", subject: "getImage" }, function (response) {
            console.log("Image Path from Content Script:", response.imagePath);
        });
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
	...
	if (imageUrl) {
		console.log("loaded an actual image");
